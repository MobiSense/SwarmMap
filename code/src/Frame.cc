...
//Frame::Frame(const KeyFrame *kf) : mpORBvocabulary(nullptr),
//                                   mTimeStamp(kf->mTimeStamp), mK(kf->mK.clone()),
//                                   mbf(kf->mbf), mb(kf->mb), mThDepth(kf->mThDepth), N(kf->N), mvKeys(kf->mvKeys),
//                                   mvKeysUn(kf->mvKeysUn), mvuRight(kf->mvuRight),
//                                   mvDepth(kf->mvDepth), mBowVec(kf->mBowVec), mFeatVec(kf->mFeatVec),
//                                   mDescriptors(kf->mDescriptors.clone()),
//                                   mvpMapPoints(kf->mvpMapPoints), mvbOutlier(vector<bool>(kf->N, false)), mnId(kf->mnId),
//                                   mpReferenceKF(const_cast<KeyFrame *>(kf)), mnScaleLevels(kf->mnScaleLevels),
//                                   mfScaleFactor(kf->mfScaleFactor), mfLogScaleFactor(kf->mfLogScaleFactor),
//                                   mvScaleFactors(kf->mvScaleFactors),
//                                   mvLevelSigma2(kf->mvLevelSigma2), mvInvLevelSigma2(kf->mvInvLevelSigma2) {
//    for (int i = 0; i < FRAME_GRID_COLS; i++) {
//        for (int j = 0; j < FRAME_GRID_ROWS; j++) {
...
bool Frame::isInFrustum(MapPoint *pMP, float viewingCosLimit)
{
   ...
   // Check viewing angle
    ...
    // TODO(halcao): update mappoint by resending?
//    pMP->SetSerialized(true);

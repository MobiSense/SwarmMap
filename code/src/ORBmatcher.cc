...
//    vector<int> rotHist[HISTO_LENGTH];
    ...
    int ORBmatcher::SearchByProjection(Frame &currentFrame, KeyFrame *pKF, const set<MapPoint *> &sAlreadyFound,
                                       const float th, const int ORBdist, const bool bGlobal) const {
        int nmatches = 0;
...
//    vector<int> rotHist[HISTO_LENGTH];
        ...
        for (size_t i = 0, iend = vpMPs.size(); i < iend; i++) {
            ...
            //Project
            cv::Mat x3Dw = bGlobal ? pMP->GetGlobalPos() : pMP->GetWorldPos();
            cv::Mat x3Dc = Rcw * x3Dw + tcw;
        ...
        if (mbCheckOrientation) {
            ...
            ORBmatcher::ComputeThreeMaxima(rotHist, HISTO_LENGTH, ind1, ind2, ind3);

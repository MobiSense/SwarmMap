...
//    cv::Mat System::GetCurrentLocation() {
//        return mpTracker->mCurrentFrame.mTcw;
//    }

    SystemState System::GetSystemState() {
        // STS: SLAM-Specific Task Scheduling
        size_t lostCount = std::count(mpTracker->mlbLost.begin(), mpTracker->mlbLost.end(), true);

//        cv::Mat velocity;
//        if (!(mpTracker->mLastFrame.mTcw.empty()) && !(mpTracker->mCurrentFrame.mTcw.empty())) {
//            cv::Mat lastTwc = cv::Mat::eye(4, 4, CV_32F);
//            mpTracker->mLastFrame.GetRotationInverse().copyTo(lastTwc.rowRange(0, 3).colRange(0, 3));
//            mpTracker->mLastFrame.GetCameraCenter().copyTo(lastTwc.rowRange(0, 3).col(3));
//            velocity = mpTracker->mCurrentFrame.mTcw * lastTwc;
//        }

        uint8_t nTracked = mpTracker->GetTrackedMapPointsNumber();
        bool bStable = mpTracker->mState == TRACKING_OK;
        bool bVelocityBurst = mpTracker->mbVelocityBurst;

        SystemState state {
            .location = mpTracker->mCurrentFrame.mTcw,
            .bVelocityBurst = bVelocityBurst,
            .bStable = bStable,
            .nTracked = nTracked,
            .lostCount = lostCount
        };

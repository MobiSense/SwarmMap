...
namespace ORB_SLAM2 {
    DynamicExtractor::DynamicExtractor(const std::string &strModelPath, int maxUsage, bool useOpticalFlow,
    ...
    void DynamicExtractor::extractMaskDirect(const Mat &frame, Mat &dynamic_mask) {
        dynamic_mask = (extractMask(frame)).clone();
    }

    cv::Mat DynamicExtractor::extractMask(const Mat &frame) {
...
//        blobFromImages()
    ...
    void DynamicExtractor::extractMaskDirect(const vector<cv::Mat> &frames, vector<cv::Mat> &masks) {
